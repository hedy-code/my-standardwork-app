    ...
    return (
      <div className="flex items-center space-x-2">
        ...
        <button 
          ...
          className="px-4 py-2 bg-slate-800 text-white font-medium rounded-lg shadow-sm hover:bg-slate-900 transition"
        >
          邮箱登录
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden p-6 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-md hover:shadow-lg transition-all active:scale-[0.98] disabled:opacity-50"
          >
            {authLoading ? '正在处理...' : (isLogin ? '立即登录' : '注册并登录')}
          </button>
        </form>
...
function App() {
  const { user, signInWithGoogle, signUpWithEmail, signInWithEmail, signOut } = useAuth();
